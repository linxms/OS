...
因为在操作系统内核中没有标准库的支持，我们需要利用alloc库定义的接口实现基本的动态内存分配器。由于内存分配器的实现比较复杂，直接使用已有的的伙伴分配器。

首先，在`Cargo.toml`配置文件中增加依赖。

```rust
buddy_system_allocator = "0.6"
```

同时，还需要在`main.rs`中引入alloc库的依赖，增加如下代码：
...
在`os/src/mm/heap_allocator.rs`中实现
...
定义页表项之前需要了解SV39分页模式下页表项每一位的含义。为了实现页表项的定义，我们需要使用Rust中常用的比特标记位的[crate bitflags]。

首先，实现页表项中的标志位PTEFlags。
在`os/src/main.rs`中实现:
...
在`os/src/linker.ld`中，`ekernel`确定了内核数据的终止地址，该地址之后的物理内存都是可用的。我们通过在config子模块设置参数来限制物理内存的大小。

在`os/src/config.rs`中实现:
...
（3）增加sync模块
此外，还需要增加sync模块，实现UPSafeCell。

在`os/src/sync/up.rs`中实现:
...
创建`os/src/mm/memory_set.rs`并实现:
...
其中VPNRange是一段虚拟页号的连续空间。在`address`子模块中实现。

接着，用MapType描述逻辑段内所有虚拟页号映射到物理页帧的方式。
修改 `os/src/mm/memory_set.rs`，具体增加如下代码:
...
因为可以直接使用ELF格式的可执行文件了，因此我们修改精简`loader`子模块。
修改`os/src/loader.rs`:
...
因为使用xmas-elf，所以需要在配置文件`Cargo.toml`增加依赖。

```toml
xmas-elf = "0.7.0"
```

最后，需要注意实现`memory_set`子模块，还需要增加如下代码：

在`os/src/mm/memory_set.rs`中实现:
...
在内核初始化的时候，需要将所有的应用程序加载到全局应用管理器中。同时，也要修改`TaskManager`的实现。
...
首先修改`init`函数。然后再`trap_handler`的开头增加`set_kernel_trap_entry`。同时，在处理完trap后还要调用`trap_return`` 返回用户态。
...
由于地址空间的隔离，`sys_write`无法直接方法应用空间的数据。为此，页表`page_table`提供一个将应用地址空间的缓冲区转化为内核地址空间可以直接访问的辅助函数。
...
从而，可以修改`sys_write`系统调用。
修改`os/src/syscall/fs.rs`:
...
除了删除`clear_bss()`的实现外，注意`删除_start()中调用的clear_bss()`。

#### (2)删除build.py

因为应用程序的起始地址是相同的了，所以不在需要build.py，直接删除即可。同时，修改`Makefile`文件。

...
> 配置完成Makefile文件后，直接执行make run操作就可以运行自己实现的系统了。同时Makefile还包含其他的内容，请自行阅读Makefile文件理解。

### 8. 思考并回答问题
（1）分析linker.ld和entry.asm所完成的功能；
![Alt text](../image/EXP02/4.png)
linker.ld 规定了程序的存储布局，特别是规定了0x80200000这个配置入口，与先前未使用自定 linker.ld 时相比，入口地址设定为了 0x80200000，使得 bootloader 能够在这个内存地址找到我们的程序。

而 entry.asm 负责在无操作系统支持的环境（裸机环境）下调用我们用 rust 编写的主函数。

（2）分析sbi模块和lang_items模块所完成的功能；

sbi模块内部封装了程序的读取字符、存入字符、以及程序的停止方法的sbi调用，便于main.rs进行调用

lang_items模块中定义了一个函数，用于打印 Panicked 的相关信息。
